...
def to_image(vertices, img_height, img_width, target_height,uni_height=True):
    ...
    if uni_height==False:
        ...
        image_vertices[:,0] = image_vertices[:,0]*img_height/2
        image_vertices[:,1] = image_vertices[:,1]*img_height/2
        # move to center of image
        image_vertices[:,0] = image_vertices[:,0] + img_width/2
        image_vertices[:,1] = image_vertices[:,1] + img_height/2
        ...
        ratio = (x.max()-x.min())/(y.max()-y.min())#模型的宽(x)和高(z)之比
        ...
        y=np.interp(y, (y.min(), y.max()),((img_height-target_height)/2, img_height-(img_height-target_height)/2))
       
        image_vertices[:,0]=x
        image_vertices[:,1]=y
    # flip vertices along y-axis.
    image_vertices[:,0] = img_height - image_vertices[:,0] - 1
