...
def compute_normals(vertex, faces):
    ...
    # 原代码：normals = normals / np.repmat(d, 3, 1)
    # 使用 np.tile 替代 np.repmat
    tiled_d = np.tile(d, (3, 1))
    normals = normals / tiled_d
...
def perform_mesh_smoothing(face, vertex, f, naver):
    ...
    for k in range(naver):
        ...
        if W.shape[1] != f.shape[0]:
            print("Dimension mismatch detected. Trying to reshape f.")
            try:
                # 尝试将 f 调整为与 W 的列数一致
                f = f.reshape(-1)
                if f.shape[0] != W.shape[1]:
                    # 如果调整后仍然不匹配，截取或填充 f 以匹配 W 的列数
                    if f.shape[0] > W.shape[1]:
                        f = f[:W.shape[1]]
                    else:
                        padding = np.zeros(W.shape[1] - f.shape[0])
                        f = np.concatenate((f, padding))
            except Exception as e:
                print(f"Failed to reshape f: {e}")
                continue
        f = W * f
...
def compute_curvature(vertex, faces):
    ...
    i, j, s = i[idx].astype(int), j[idx].astype(int), s[idx].astype(int)
    s += 1
    ...
    for x in range(3):
        for y in range(3):
            ...
            # 原代码：Tv[x, y, :] = np.reshape(a, (1, 1, n))
            # 修改为检查数组大小并调整
            if a.size == n:
                Tv[x, y, :] = np.reshape(a, (1, 1, n))
            else:
                print(f"警告: 数组大小 {a.size} 与目标形状 (1, 1, {n}) 不兼容，跳过重塑操作。")
